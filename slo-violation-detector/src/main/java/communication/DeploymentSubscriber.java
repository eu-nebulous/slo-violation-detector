...
import static slo_violation_detector_engine.detector.DetectorSubcomponentState.deploymentTimeRecordingQueueLock;
    ...
    public DeploymentSubscriber(String broker_ip_address, int broker_port, String broker_username, String broker_password, DetectorSubcomponent detector){
        ...
        BiFunction<BrokerSubscriptionDetails,String,Long> function = (broker_details, message) ->{
            ...
            synchronized (deploymentTimeRecordingQueueLock) {
                try {
                    deployment_timestamp = System.currentTimeMillis();
                    //detector.getSubcomponent_state().getReconfiguration_time_recording_queue().add(deployment_timestamp);
                    detector.getSubcomponent_state().getDeployment_timestamps().add(deployment_timestamp);
                } catch (Exception e){
                    e.printStackTrace();
